...
MySQL - 5.6.15 : Database - secureusers
*********************************************************************
*/
drop database `secureusers`;
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`secureusers` /*!40100 DEFAULT CHARACTER SET utf8 */;
...
insert  into `users`(`username`,`password`,`locked`) values ('adam','adam','0'),('bill','bill','0');
...
BEGIN
#Trial calls to this procedure:
#CALL getUser('adam');
#CALL getUser("' or 1=1;#");
	
	set @sqlStmt = concat('select * from users where username=''',theUsername,''';');
	
	#OR: use 0x27 to hide obvious quotes...
	#set @sqlStmt = concat('select * from users where username=',0x27,theUsername,0x27,';');
	
	prepare aUser from @sqlStmt;
		
	execute aUser;
	
	deallocate prepare aUser;
    END */$$
DELIMITER ;
